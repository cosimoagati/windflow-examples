...
static Metric<unsigned long> global_latency_metric {"tt-threads-latency"};
#ifndef NDEBUG
...
public:
    RollingCounterFunctor(unsigned emit_frequency_in_seconds = 60,
    ...
    RollingCounterFunctor(const RollingCounterFunctor &other)
...
public:
    RankerFunctor(unsigned emit_frequency_in_seconds = 60, unsigned count = 10)
        : time_units_between_ticks {emit_frequency_in_seconds
                                    * timeunit_scale_factor},
          count {count} {}
...
get_stas_with_emit_freqs(const nlohmann::ordered_json &json_stats,
...
int main(int argc, char *argv[]) {
    ...
    const auto latency_stats = get_stas_with_emit_freqs(
        get_distribution_stats(global_latency_metric, parameters,
                               global_received_tuples),
        parameters);
    serialize_json(latency_stats, "tt-threads-latency",
                   parameters.metric_output_directory);

    const auto throughput_stats = get_stas_with_emit_freqs(
        get_single_value_stats(throughput, "throughput", parameters,
                               global_sent_tuples),
        parameters);
    serialize_json(throughput_stats, "tt-threads-throughput",
                   parameters.metric_output_directory);

    const auto service_time_stats = get_stas_with_emit_freqs(
